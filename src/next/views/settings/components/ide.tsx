...
import { VscodeIcon } from "@next/components/icons";
...
export default function IDESettings() {
    ...
    const onSubmit = methods.handleSubmit(async (data) => {
        try {
            ...
            else await window.electron.db.save("Setting", { key: SETTINGS.ide, value: data.command });

            toast.success("IDE settings saved!")
        }
        catch (err) {
    ...
    return <Box sx={{ my: 4 }}>
        <Stack gap={1}>
            ...
            <Form methods={methods} onSubmit={onSubmit}>
                <RHFSingle name="command" icon={<VscodeIcon />} />
            </Form>
            <Typography variant="caption" color="text.secondary" >
                the command you put in here must include "$1" variable to let app pass path to your IDE.
            </Typography>
        </Stack>
    </Box>
}