...
export default function RunButton({ command, size = 18, title }: Props) {
    ...
    const handleClose = async (e: React.MouseEvent<HTMLLIElement, MouseEvent>) => {
        setAnchorEl(null);
        try {
            const bash = await window.electron.terminal.createBash(command);
            ...
            switch ((e.target as any).dataset.id) {
                case "internal-proxied":
                    create(title, `proxychains4 ${bash}`)
                    break;
                case "external":
                    if (terminalExternal) {
    ...
    return <>
        <IconButton size="small" aria-label="run script" onClick={handleClick}>
            <TerminalIcon width={size} height={size} />
        </IconButton>
        <Menu
            anchorEl={anchorEl}
            open={open}
